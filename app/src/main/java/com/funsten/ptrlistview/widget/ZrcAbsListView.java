...
import com.funsten.ptrlistview.R;
import com.funsten.ptrlistview.utils.APIUtil;
import com.funsten.ptrlistview.widget.ZrcListView.OnScrollListener;
import com.funsten.ptrlistview.widget.ZrcListView.OnScrollStateListener;
import com.funsten.ptrlistview.widget.ZrcListView.OnStartListener;
	...
	protected int computeVerticalScrollOffset() {
		...
		if (firstPosition >= 0 && childCount > 0) {
			if (mSmoothScrollbarEnabled) {
				...
				if (height > 0) {
					return Math.max((firstPosition * range - top * range / height) / count + mFirstTopOffset * 10 + mScrollY * 10, mFirstTopOffset * 10);
				}
			} else {
				int index;
				final int count = mItemCount;
		...
		if (childCount > 0 && (isRefreshing || isLoadingMore) && footer != null) {
			...
			if (firstPosition + childCount == mItemCount) {
				...
				if (lastBottom < viewBottom) {
					boolean more = footer.draw(canvas, listPadding.left, lastBottom, getWidth() - listPadding.right, isTooShort ? lastBottom + footer.getHeight() : viewBottom - mLastBottomOffset);
					if (more) {
